...
import { useShoppingCartLazyQuery } from '../hooks/useShoppingCartContext';
...
const trackProductFullDetail = orig => {
  return function useProductFullDetail(props) {
    ...
    const addToCartFlag = useRef(false);
    // const [getCart, { data, loading, called }] = useShoppingCartLazyQuery();
    ...
    // useEffect(() => {
    //   if ((addToCartFlag.current = true)) {
    //     console.log(data);
    //     console.log('pdp add to cart');
    //     mdl.publish.addToCart();
    //     addToCartFlag.current = false;
    //   }
    // }, [addToCartFlag]);
