...
const useCollector = () => {
  const defaultMagentoStoreEvents = useCallback(() => {
    ...
    magentoStoreEvents('newTracker', 'magento-storefront-js', process.env.SNOWPLOW_URL, {
  ...
  useEffect(() => {
    ...
    window[collectorName] = function () {
      (window[collectorName].q = window[collectorName].q || []).push(arguments);
    };
    ...
    import('../src/sp')
      ...
      .catch(e => {
        console.error('Product Recommendations Collector script failed to load:', e);
      });
