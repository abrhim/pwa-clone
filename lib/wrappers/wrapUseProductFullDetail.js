...
import useProductDataLayer from '../collectors/useProductDataLayer';
...
const trackProductFullDetail = orig => {
  return function useProductFullDetail(props) {
    ...
    const { pushProductPageView, resetProductPageView } = useProductDataLayer({ product });

    const { pushProductAddToCart } = useProductDataLayer({
      product,
      productSku,
    });
